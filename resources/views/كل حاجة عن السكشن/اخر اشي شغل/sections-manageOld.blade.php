    ...
    <div class="main-content">
        <div class="data-entry-container">
            ...
            <div class="mb-4 p-3 border rounded bg-light shadow-sm">
                ...
                @else
                    <div class="alert alert-warning small mt-2 p-2">Expected student count not found. Automatic generation
                        may be inaccurate. <a href="{{ route('data-entry.plan-expected-counts.index') }}"
                            class="alert-link">Add it here.</a></div>
                @endif
            </div>
            ...
            @php
                ...
                if ($subject && $subject->subjectCategory) {
                    ...
                    // $currentSections يتم تمريرها من الكنترولر وتحتوي على كل شعب المادة في هذا السياق
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <button
                                                            ...
                                                            data-branch="{{ $section->branch }}"
                                                            data-activity-type="{{ $section->activity_type }}"
                                                            data-context-info="Sec #{{ $section->section_number }} for {{ optional($planSubject->subject)->subject_no }} (Practical)">Edit</button>
                                                        <button
                                                            class="btn btn-sm btn-outline-danger py-0 px-1 open-delete-section-modal"
                                                            data-bs-toggle="modal" data-bs-target="#deleteSectionModal"
                                                            data-form-action="{{ route('data-entry.sections.destroy', $section->id) }}"
                                                            data-section-info="Section #{{ $section->section_number }} (Practical) for {{ optional($planSubject->subject)->subject_no }}">Delete</button>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
            ...
            {{-- *** تضمين المودالات في نهاية الصفحة *** --}}
            @include('dashboard.data-entry.partials._sections_modals')
            {{-- لا حاجة لتمرير section هنا لأن المودالات عامة ويتم تعبئتها بـ JS --}}
