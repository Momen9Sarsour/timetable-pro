                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    {{-- // زر التعديل --}}
                                    <button class="btn btn-sm btn-outline-primary me-1" data-bs-toggle="modal" data-bs-target="#editRoomModal-{{ $room->id }}" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    {{-- // زر الحذف --}}
                                    <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteRoomModal-{{ $room->id }}" title="Delete">
                                        <i class="fas fa-trash"></i>
                                    </button>

                                    {{-- // تضمين Modals التعديل والحذف الخاصة بهذه القاعة --}}
                                    @include('dashboard.data-entry.partials._room_modals', ['room' => $room, 'roomTypes' => $roomTypes]) {{-- // نمرر أنواع القاعات أيضاً --}}

                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="9" class="text-center text-muted">No classrooms found. Click 'Add New Classroom' to create one.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
