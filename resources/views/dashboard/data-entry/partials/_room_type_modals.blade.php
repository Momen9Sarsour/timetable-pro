        ...
        <div class="modal-content">
            ...
            <form action="{{ route('data-entry.room-types.store') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                         {{-- // تغيير الحقول --}}
                        <label for="add_room_type_name" class="form-label">Room Type Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('room_type_name', 'store') is-invalid @enderror" id="add_room_type_name" name="room_type_name" value="{{ old('room_type_name') }}" required>
                        @error('room_type_name', 'store') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                </div>
...
<div class="modal fade" id="editRoomTypeModal-{{ $roomType->id }}" tabindex="-1" aria-labelledby="editRoomTypeModalLabel-{{ $roomType->id }}" aria-hidden="true">
     <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editRoomTypeModalLabel-{{ $roomType->id }}">Edit Room Type: {{ $roomType->room_type_name }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
             ...
             <form action="{{ route('data-entry.room-types.update', $roomType->id) }}" method="POST">
                ...
                <div class="modal-body">
                     <div class="mb-3">
                        ...
                        <label for="edit_room_type_name_{{ $roomType->id }}" class="form-label">Room Type Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('room_type_name', 'update_'.$roomType->id) is-invalid @enderror" id="edit_room_type_name_{{ $roomType->id }}" name="room_type_name" value="{{ old('room_type_name', $roomType->room_type_name) }}" required>
                        @error('room_type_name', 'update_'.$roomType->id) <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        ...
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
             ...
             <form action="{{ route('data-entry.room-types.destroy', $roomType->id) }}" method="POST">
                ...
                <div class="modal-body">
                    {{-- // تغيير النص والمتغير --}}
                    <p>Are you sure you want to delete the room type: <strong>{{ $roomType->room_type_name }}</strong>?</p>
