...
@section('content')
                    ...
                    <div class="schedule-body">
                        @foreach ($groups as $groupKey => $group)
                            <div class="group-row" data-group-key="{{ $groupKey }}">
                                ...
                                @foreach ($days as $dayIndex => $dayName)
                                    <div class="day-sessions-container schedule-drop-zone" data-day="{{ $dayIndex }}"
                                        ...
                                        @foreach ($daySessions as $session)
                                            ...
                                            <div class="session-block draggable-course {{ $session->activity_type === 'Theory' ? 'theory' : 'practical' }}"
                                                ...
                                                data-group-key="{{ $groupKey }}" data-start="{{ $startOffset }}"
                                                data-end="{{ $endOffset }}" data-subject="{{ $session->subject_name }}"
                                                ...
                                                <div class="session-info">
                                                    <!-- Editable Instructor -->
                                                    <div class="editable-field" data-field="instructor"
                                                        data-gene-id="{{ $session->gene_id }}"
                                                    ...
                                                    <!-- Editable Room -->
                                                    <div class="editable-field" data-field="room"
                                                        data-gene-id="{{ $session->gene_id }}"
                                                    ...
                                                    <div><i class="fas fa-clock"></i> {{ $session->start_time }} -
    ...
    <!-- Changes Indicator -->
    <div id="changesIndicator" class="position-fixed bottom-0 start-50 translate-middle-x mb-3 d-none"
        style="z-index: 1050;">
    ...
    <div id="loadingOverlay" class="position-fixed top-0 start-0 w-100 h-100 d-none"
        style="background: rgba(255,255,255,0.95); z-index: 10000;">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function createEditSelect(field, fieldType, geneId, currentValue, currentText, options) {
                ...
                $(select).on('select2:open', function() {
                    // ركّز على حقل البحث بعد فتح القائمة
            ...
            function updateFieldValue(select, fieldType, geneId, newText, newValue) {
                ...
                // ✅ تأكد من إضافة البيانات بشكل صحيح
                pendingChanges.push({
                    type: 'edit',
                    gene_id: parseInt(geneId), // ✅ تحويل لـ number
                    field: fieldType,
                    new_value_id: parseInt(newValue), // ✅ تحويل لـ number
                    newText: newText,
                    oldValue: parseInt(oldValue), // ✅ تحويل لـ number
                    oldText: oldText,
                    timestamp: Date.now()
                });

                console.log('📝 Edit added:', { // ✅ للتأكد من البيانات
                    gene_id: geneId,
                    field: fieldType,
            ...
            function revertToOriginalField(select, fieldType) {
                const div = createFieldDiv(fieldType, select.dataset.geneId, select.dataset.originalText, select
                    .dataset);
                $(select).replaceWith(div);
            }
            ...
            async function performSave() {
                ...
                // ✅ Log للتأكد من البيانات قبل الإرسال
                ...
                try {
                    ...
                    console.log('📥 Server response:', result); // ✅ Log للرد من السيرفر
            ...
            window.addEventListener('beforeunload', function(e) {
                if (pendingChanges.length > 0) {
                    const confirmationMessage =
                        `You have ${pendingChanges.length} unsaved changes. Are you sure you want to leave?`;
